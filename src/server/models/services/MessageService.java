...
public class MessageService {
    ...
    public static String selectAllInto(List<Message> targetList) {
        ...
        try {
            PreparedStatement statement = DatabaseConnection.newStatement(
                    "SELECT Id, Text, PostDate, Author, Board FROM Messages"
            );
            if (statement != null) {
                ResultSet results = statement.executeQuery();
                if (results != null) {
                    while (results.next()) {
                        targetList.add(new Message(results.getInt("Id"), results.getString("Text"), results.getString("PostDate"), results.getString("Author"), results.getInt("Board")));
    ...
    public static Message selectById(int id) {
        ...
        try {
            PreparedStatement statement = DatabaseConnection.newStatement(
                    "SELECT Id, Text, PostDate, Author, Board FROM Messages WHERE Id = ?"
            );
            if (statement != null) {
                statement.setInt(1, id);
                ResultSet results = statement.executeQuery();
                if (results != null && results.next()) {
                    result = new Message(results.getInt("Id"), results.getString("Text"), results.getString("PostDate"), results.getString("Author"), results.getInt("Board"));
    ...
    public static String insert(Message itemToSave) {
        try {
            PreparedStatement statement = DatabaseConnection.newStatement(
                    "INSERT INTO Messages (Id, Text, PostDate, Author, Board) VALUES (?, ?, ?, ?, ?)"
            );
            ...
            statement.setInt(5, itemToSave.getBoard());
    ...
    public static String update(Message itemToSave) {
        try {
            PreparedStatement statement = DatabaseConnection.newStatement(
                    "UPDATE Messages SET Text = ?, PostDate = ?, Author = ?, Board = ? WHERE Id = ?"
            );
            ...
            statement.setInt(4, itemToSave.getBoard());
